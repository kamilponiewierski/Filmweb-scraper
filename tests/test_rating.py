...
class TestRating(TestCase):
    def test_create_rating_from_match(self):
        with open("sample_page", encoding='utf-8') as test_page:
            text = test_page.read()
            matches = filmweb.Rating.regex.finditer(text)
            for match in matches:
                movie = filmweb.Rating.create_rating_from_match(match)
    ...
    def test_to_csv(self):
        test = filmweb.Rating(filmweb.Movie("Niebo i piekło", "1963"), "8,0", "2 241", "17 września 2018", 9)
